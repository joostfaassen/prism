...
{
    /**
     * @param array<string, array<string, mixed>> $profiles Profile configs keyed by name
    ...
    public function getProfilesByType(string $type): array
    {
        return array_filter(
            $this->profiles,
            fn(array $profile) => ($profile['type'] ?? '') === $type,
        );
    }

    /**
     * @return list<string> Unique profile types present in this server
     */
    ...
    {
        return in_array($type, $this->getProfileTypes(), true);
    }

    /**
     * Decide whether a tool is exposed on this server.
     *
     * Profiles may declare an optional `tools` allowlist (a list of tool-name
     * patterns, with `*`/`?` wildcards) to restrict which of their type's tools
     * are exposed. The allowlist is collected across every profile of the tool's
     * type:
     *
     *  - If no profile of that type declares a `tools` list, all of that type's
     ...
     *
     * Utility tools (those without a profile type) are always allowed.
     */
    public function isToolAllowed(string $toolName, ?string $profileType): bool
    {
        if ($profileType === null) {
            return true;
        }
        ...
        foreach ($this->getProfilesByType($profileType) as $profile) {
            if (!array_key_exists('tools', $profile)) {
                continue;
            }

            $hasAllowlist = true;
            foreach ((array) $profile['tools'] as $pattern) {
