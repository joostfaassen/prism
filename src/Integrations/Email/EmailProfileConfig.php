...
/**
 * A unified email profile: IMAP for reading + writing to folders (e.g. Sent),
 * SMTP for outbound delivery, and the identity used as the "From" address.
 *
 * SMTP and identity are optional. A profile that only has IMAP configured can
 * still be used for read-only tools, but `email_send` will refuse to use it.
 */
class EmailProfileConfig
{
